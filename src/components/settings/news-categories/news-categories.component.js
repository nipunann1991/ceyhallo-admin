var __decorate = (this && this.__decorate) || function (decorators, target, key, desc) {
    ...
    if (typeof Reflect === "object" && typeof Reflect.decorate === "function") r = Reflect.decorate(target, key, desc, d);
...
let NewsCategoriesComponent = class NewsCategoriesComponent {
    constructor() {
        this.authService = inject(AuthService);
        this.toastService = inject(ToastService);
        this.newsCategoriesService = inject(NewsCategoriesService);
        this.lastGeneratedAddSlug = '';
        this.categories = this.newsCategoriesService.categories;
        this.newCategoryName = signal('');
        this.newCategorySlug = signal('');
        this.editingId = signal(null);
        this.editingName = signal('');
        this.editingSlug = signal('');
        this.categoryToDelete = signal(null);
        this.showDeleteModal = signal(false);
        this.deleteMessage = computed(() => `Are you sure you want to delete '${this.categoryToDelete()?.name || ''}'?`);
    }
    async addCategory() {
    ...
    onNewSlugInput(event) {
        this.newCategorySlug.set(this.slugify(event.target?.value || ''));
    }
    onEditSlugInput(event) {
        this.editingSlug.set(this.slugify(event.target?.value || ''));
    }
    slugify(value) {
        return String(value || '')
            .toLowerCase()
...
NewsCategoriesComponent = __decorate([
    Component({
...
export { NewsCategoriesComponent };
