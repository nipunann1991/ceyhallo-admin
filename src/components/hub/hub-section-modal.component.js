...
let HubSectionModalComponent = class HubSectionModalComponent {
    constructor() {
        this.section = input(null);
        this.countries = input([]);
        this.defaultCountry = input('AE');
        this.close = output();
        this.saved = output();
    ...
    get isTitleVisibleControl() {
        return this.form.get('isTitleVisible');
    }
    ngOnInit() {
        ...
        if (s) {
            ...
            const titleVis = s.isTitleVisible !== undefined ? s.isTitleVisible : s.isVisible;
        ...
        this.firebaseService.listenToPath('categories', (data) => {
            const filteredData = data.filter((cat) => cat.label !== 'Popular' && cat.label !== 'Featured');
            const sorted = filteredData.sort((a, b) => a.label.localeCompare(b.label));
...
HubSectionModalComponent = __decorate([
    Component({
...
], HubSectionModalComponent);
export { HubSectionModalComponent };
