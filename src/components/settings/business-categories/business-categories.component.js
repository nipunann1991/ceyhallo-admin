...
let BusinessCategoriesComponent = class BusinessCategoriesComponent {
    constructor() {
        this.firebaseService = inject(FirebaseService);
        this.toastService = inject(ToastService);
        this.excludedCategoriesService = inject(ExcludedCategoriesService);
        this.categories = signal([]);
        this.excludedCategories = this.excludedCategoriesService.excludedCategories; // Use the service's signal
        this.showModal = signal(false);
        this.showConfirmModal = signal(false);
        this.selectedCategory = signal(null);
        this.confirmMessage = computed(() => `Are you sure you want to delete the category '${this.selectedCategory()?.name}'?`);
    ...
    async toggleCategoryExclusion(category) {
    ...
    onDrop(event, dropIndex) {
        ...
        const updates = cats.map((cat, index) => this.firebaseService.update('taxonomy_business', cat.id, { order: index }));
...
BusinessCategoriesComponent = __decorate([
    Component({
...
export { BusinessCategoriesComponent };
